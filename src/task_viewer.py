...
def time_unit_converter(sentence):
    ...
    time_unit = ["v hari", "hari ini", "besok", "lusa", "x minggu", "minggu depan", "y bulan", "bulan depan", "z tahun", "tahun depan"]
...
def task_viewer(sentence):
    ...
    # db = loadTask()
    db = [["10/12/2021", "IF9999", "Kuis", "PlaceHolder Task", False], ["12/12/2021", "IF9997", "Tucil", "PlaceHolder Task", False], ["11/12/2021", "IF9998", "Tubes", "PlaceHolder Task", False]]
    ...
    if (boyer_moore_string_matching(sentence, "deadline")):
        ...
        tugas_in_match = [match[i] for i in range(len(match)) if match[i] in ["Tubes", "Tucil"]]
        if (tugas_in_match == []):
